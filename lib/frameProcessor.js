...
import { analyzeRotation } from './rotationEstimator';
import { analyzeGaze } from './gazeEstimator';
...
export function createFrameProcessor(options) {
    ...
    async function processFrame() {
        ...
        try {
            ...
            // Analyze rotation and gaze if face detected
            let isRotated = false;
            let rotationDetails = null;
            let isLookingAway = false;
            let gazeDetails = null;
            ...
            if (detection.count === 1) {
                ...
                rotationDetails = analyzeRotation(landmarks);
                isRotated = rotationDetails.isRotated;

                // Analyze eye gaze (only if head is not rotated)
                if (!isRotated) {
                    gazeDetails = analyzeGaze(landmarks);
                    isLookingAway = gazeDetails.isLookingAway;
                }
            }
            ...
            if (onAnalysis) {
                onAnalysis({
                    faceCount: detection.count,
                    faces: detection.faces,
                    isRotated,
                    rotationDetails,
                    isLookingAway,
                    gazeDirection: gazeDetails?.gazeDirection || null,
                    gazeDetails,
                    brightness,
                    processingTime
                });
